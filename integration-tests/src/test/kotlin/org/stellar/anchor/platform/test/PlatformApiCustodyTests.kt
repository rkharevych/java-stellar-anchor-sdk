...
class PlatformApiCustodyTests(config: TestConfig, toml: Sep1Helper.TomlContent, jwt: String) {
  ...
  private fun `test flow`(txId: String, actionRequests: String, actionResponses: String) {
    val rpcRequestsType = object : TypeToken<List<RpcRequest>>() {}.type
    val rpcRequests: List<RpcRequest> =
      gson.fromJson(actionRequests.replace(TX_ID_KEY, txId), rpcRequestsType)

    val rpcResponses = platformApiClient.callRpcRequest(rpcRequests)

    val expectedResult = actionResponses.replace(TX_ID_KEY, txId).trimIndent()
    val actualResult = rpcResponses.body?.string()?.trimIndent()
