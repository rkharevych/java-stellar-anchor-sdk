...
import org.stellar.anchor.api.sep.SepTransactionStatus.PENDING_SENDER
...
class Sep24ActionsEnd2EndTests(
  config: TestConfig,
  ...
  private fun `test typical receive end-to-end flow`(asset: StellarAssetId, amount: String) =
    runBlocking {
      ...
      anchorReferenceServerClient.sep31Receive(postTxResponse.id)
      ...
      assertEquals(PENDING_SENDER, transaction.status)
      ...
      val transfer =
        wallet
          .stellar()
          .transaction(keypair)
          .transfer("GBN4NNCDGJO4XW4KQU3CBIESUJWFVBUZPOKUZHT7W7WRB7CWOA7BXVQF", asset, amount)
          .addMemo(Pair(MemoType.HASH, transaction.memo))
          .build()
      transfer.sign(keypair)
      wallet.stellar().submitTransaction(transfer)
  ...
  private val expectedDepositEventsJson =
    """
[
  {
